 ...
 * such creates (ConditionalPageModule for `POST /api/pages/idempotent-create`, ServiceBridgeModule for the keyed
 * `POST /api/service/spaces`) — never by an upstream file. Nest instantiates it once however many modules import
 * it, so the installer and the sweep run once per process.
