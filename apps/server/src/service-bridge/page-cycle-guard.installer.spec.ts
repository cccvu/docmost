...
describe('PageCycleGuardInstaller', () => {
  const ENV = ['PAGE_CYCLE_GUARD_INSTALL_MAX_ATTEMPTS', 'PAGE_CYCLE_GUARD_INSTALL_RETRY_MS'] as const;
  ...
  it('installs NOTHING in native mode', async () => {
    const spy = spyKysely(() => []);
    await new PageCycleGuardInstaller(spy.db, 'native' as any).onApplicationBootstrap();
  ...
  it('installs the guard function + BEFORE UPDATE OF parent_page_id trigger in ONE advisory-locked transaction', async () => {
    const spy = spyKysely(() => []);
    await new PageCycleGuardInstaller(spy.db, 'remote' as any).onApplicationBootstrap();
    ...
    expect(all).toMatch(/pg_advisory_xact_lock\(485485, hashtext\(new\.workspace_id::text\)\)/);
  ...
  it('RETRIES while the engine tables are not ready, then succeeds', async () => {
    ...
    const spy = spyKysely((q) => {
      if (!failedOnce && q.sql.includes('create trigger ccc_page_cycle_guard')) {
        failedOnce = true;
        throw new Error('relation "pages" does not exist');
      }
      return [];
    });
    await expect(new PageCycleGuardInstaller(spy.db, 'remote' as any).onApplicationBootstrap()).resolves.toBeUndefined();
  ...
  it('REFUSES TO BOOT (fail-closed) in remote when the install never succeeds', async () => {
    ...
    await expect(new PageCycleGuardInstaller(spy.db, 'remote' as any).onApplicationBootstrap()).rejects.toThrow(
      /page cycle guard install failed in remote mode/,
    );
